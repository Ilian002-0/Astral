...
const StatCard: React.FC<{ title: string; value: string; colorClass?: string; tooltip?: string }> = ({ title, value, colorClass = 'text-white', tooltip }) => (
  <div className="bg-gray-800/50 p-4 rounded-3xl shadow-lg border border-gray-700/50 transform transition-all duration-300 hover:scale-105 hover:bg-gray-800 group relative">
    ...
    {tooltip && (
      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max px-3 py-1.5 text-sm font-medium text-white bg-gray-900 rounded-2xl shadow-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
        {tooltip}

...
const RankBadge: React.FC<{ rank: number }> = ({ rank }) => {
    ...
    const config = {
        ...
        2: {
            ...
            shadow: 'shadow-[0_0_12px_rgba(156,163,175,0.4)]'
        },
        3: {
    ...
    return (
        <div 
            className={`flex items-center justify-center w-7 h-7 rounded-full border border-opacity-50 ${config.bgColor} ${config.borderColor} ${config.shadow} animate-spring-up`}
            title={`Rank #${rank}`}
        >
            <span className={`font-black text-sm leading-none ${config.textColor}`}>{rank}</span>
        </div>
    );
};

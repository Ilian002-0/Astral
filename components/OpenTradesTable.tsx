...
const OpenTradesTable: React.FC<OpenTradesTableProps> = ({ trades, floatingPnl }) => {
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                    <thead className="text-xs text-gray-400 uppercase border-b border-gray-700">
                        <tr>
                            <th scope="col" className="px-2 sm:px-4 py-3">{t('trades_list.col_symbol')}</th>
                            <th scope="col" className="px-2 sm:px-4 py-3">{t('trades_list.col_open_time')}</th>
                            <th scope="col" className="px-2 sm:px-4 py-3">{t('trades_list.col_type')}</th>
                            <th scope="col" className="px-2 sm:px-4 py-3 text-right">{t('trades_list.col_size')}</th>
                            <th scope="col" className="px-2 sm:px-4 py-3 text-right">{t('trades_list.col_profit')}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {trades.map((trade) => {
                            ...
                            return (
                                <tr key={trade.ticket} className="border-b border-gray-800 text-xs">
                                    ...
                                    <td className="px-2 sm:px-4 py-3 text-white">{formatDate(trade.openTime)}</td>
                     ...
                     <tfoot>
                        <tr className="font-semibold text-white">
                            <td colSpan={4} className="px-3 sm:px-6 py-4 text-base font-bold">{t('open_trades.total_floating_pnl')}</td>
                            <td className={`px-3 sm:px-6 py-4 text-right text-base font-bold ${floatingPnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>

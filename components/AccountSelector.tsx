  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center justify-between w-32 md:w-48 px-3 md:px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-2xl shadow-md transition-colors"
        aria-haspopup="true"
        aria-expanded={isOpen}
      >
        <span className="truncate text-sm md:text-base">{currentAccount || 'Select Account'}</span>
        <svg className={`w-4 h-4 md:w-5 md:h-5 ml-2 transition-transform duration-200 ${isOpen ? 'transform rotate-180' : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
